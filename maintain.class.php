 ...
 */
class SupervisorAlbum_maintain extends PluginMaintain
{
  ...
  function __construct($plugin_id)
  {
    ...
    $this->table = $prefixeTable . 'supervisor';
    $this->dir = PHPWG_ROOT_PATH . PWG_LOCAL_DIR . 'supervisor/';
  }
  ...
  function install($plugin_version, &$errors=array())
  {
    ...
    if (empty($conf['supervisor']))
    {
      ...
      conf_update_param('supervisor', $this->default_conf, true);
    }
    else
    {
      $old_conf = safe_unserialize($conf['supervisor']);



      conf_update_param('supervisor', $old_conf, true);
    }
...
CREATE TABLE IF NOT EXISTS `'. $this->table .'` (
  ...
  `champs` VARCHAR(100) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8
;');

    // add a new column to existing table
    $result = pwg_query('SHOW COLUMNS FROM `'.IMAGES_TABLE.'` LIKE "supervisor";');
    if (!pwg_db_num_rows($result))
    {
      pwg_query('ALTER TABLE `' . CATEGORIES_TABLE . '` ADD `id_supervisor` int(11) DEFAULT NULL;');
    }
  ...
  function uninstall()
  {
    ...
    conf_delete_param('supervisor');

    // delete table
    pwg_query('DROP TABLE `'. $this->table .'`;');

    // delete field
    pwg_query('ALTER TABLE `'. IMAGES_TABLE .'` DROP `id_supervisor`;');
